...
- Platform adapters live under `src/`:
  ...
  - Laravel package: [`src/laravel/`](src/laravel) with Blade component.
  - Symfony bundle: [`src/symfony/`](src/symfony) with Twig extension.
  - Drupal module: [`src/drupal/`](src/drupal) with a block plugin.
  - Joomla content plugin: [`src/joomla/`](src/joomla) replacing `{d3v-legal ...}` tags.
  - Magento 2 module: [`src/magento/`](src/magento) with a template block.
  - PrestaShop module: [`src/prestashop/`](src/prestashop) with a Smarty plugin.
  - Shared PHP renderer used by PHP adapters: [`src/php-shared/D3vLegalRenderer.php`](src/php-shared/D3vLegalRenderer.php).
...
- PHP linting: `find src -type f -name '*.php' -exec php -l {} \;`.
- Test suite: `php composer.phar test` or `vendor/bin/phpunit --configuration phpunit.xml.dist`.
- EmDash build: `cd src/emdash-plugin && npm run build`.
- JS syntax check: `node --check src/javascript-native/d3v-legal.js` and `node --check src/javascript-native/config.js`.
- Release archives are platform-specific ZIPs built by CI. Current platforms: `wordpress`, `javascript-native`, `emdash-plugin`, `laravel`, `symfony`, `drupal`, `joomla`, `magento`, `prestashop`. The `docs/` directory is excluded from release packages. Every ZIP includes a self-contained copy of `legal-libraries/`.
...
- 2026-07-29: Added shared PHP renderer (`src/php-shared/D3vLegalRenderer.php`) and adapters for Laravel, Symfony, Drupal, Joomla, Magento 2, and PrestaShop; updated the release workflow to version-sync and package all nine platforms; updated README, SDD, AGENT_MEMORY and created `docs/TODO.md`.
